...
function generateRequestFingerprint(
  ...
  const components: string[] = [request.method.toUpperCase(), request.url];
      ...
      reply.send = function (payload: unknown): FastifyReply {
        try {
          ...
          if (reply.statusCode >= 200 && reply.statusCode < 300) {
            ...
            void cache.set(cacheKey, cachedResponse, config.cacheTtlSeconds).catch((error) => {
              request.log.error(
                {
                  requestId,
                  fingerprint,
...
export function registerRequestDeduplication(
  ...
  fastify.log.info(options, 'Request deduplication middleware registered globally');
}
