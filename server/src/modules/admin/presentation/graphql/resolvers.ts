...
export const adminResolvers = {
  Query: {
    ...
    jobDashboard: async (_: any, __: any, context: AdminGraphQLContext) => {
      ...
      try {
        ...
        const completionRates = Array.from(dashboardData.metrics.completionRates.entries()).map(
          ([queueName, metrics]) => ({
            queueName,
            metrics,
          })
        ...
        const queueDepths = Array.from(dashboardData.metrics.queueDepths.entries()).map(
          ([queueName, metrics]) => ({
            queueName,
            metrics,
          })
  ...
  Mutation: {
     ...
     * Retry failed jobs
     */
    retryJobs: async (_: any, { input }: { input: any }, context: AdminGraphQLContext) => {
      const admin = requireAdmin(context);
     ...
     */
    manageQueue: async (_: any, { input }: { input: any }, context: AdminGraphQLContext) => {
      const admin = requireAdmin(context);
