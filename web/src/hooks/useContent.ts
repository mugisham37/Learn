...
import type { VideoProcessingStatus, StreamingUrl } from '../types';
import type {
...
export function useVideoUpload(): VideoUploadResult {
  ...
  const uploadVideo = useCallback(
    async (file: File, lessonId: string, options: UploadOptions = {}) => {
      const coreOptions: CoreUploadOptions = {
        ...(options.courseId && { courseId: options.courseId }),
        lessonId,
        ...(options.onProgress && {
...
export function useVideoAsset(
  videoAssetId: string
): QueryResult<GetVideoAssetResponse['videoAsset']> {
...
export function useAssetManagement(): AssetManagementResult {
  ...
  const deleteAsset = useCallback(
    async (assetId: string, assetType: 'video' | 'file' = 'file'): Promise<boolean> => {
      if (assetType === 'video') {
  ...
  const loading =
    ...
    cancelLoading;
  const error =
    deleteVideoError || deleteFileError || deleteContentError || retryError || cancelError;
