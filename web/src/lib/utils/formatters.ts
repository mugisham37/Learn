 ...
 * Used for round-trip testing and validation
 */
export const parseCurrency = (currencyString: string, locale: string = DEFAULT_LOCALE): number => {
...
export const formatDuration = memoize(
  (durationMs: number, options: DurationFormatOptions = {}): string => {
    const { format = 'short', precision = 'minutes', showZero = false } = options;
    ...
    // Calculate each unit
    for (const [unit, ms] of Object.entries(DURATION_UNITS) as Array<
      [keyof typeof DURATION_UNITS, number]
    >) {
      if (remaining >= ms) {
...
export const parseDuration = (durationString: string): number => {
  ...
  while ((match = regex.exec(durationString)) !== null) {
    ...
    const unitMs = unitLower.startsWith('y')
      ? DURATION_UNITS.year
      : unitLower.startsWith('mo')
        ? DURATION_UNITS.month
        : unitLower.startsWith('w')
          ? DURATION_UNITS.week
          : unitLower.startsWith('d')
            ? DURATION_UNITS.day
            : unitLower.startsWith('h')
              ? DURATION_UNITS.hour
              : unitLower.startsWith('m')
                ? DURATION_UNITS.minute
                : unitLower.startsWith('s')
 ...
 */
export const formatNumber = memoize((number: number, options: NumberFormatOptions = {}): string => {
  const {
    locale = DEFAULT_LOCALE,
...
export const formatPercentage = memoize(
  (value: number, options: Omit<NumberFormatOptions, 'style'> = {}): string => {
 ...
 */
export const formatFileSize = memoize((bytes: number, locale: string = DEFAULT_LOCALE): string => {
