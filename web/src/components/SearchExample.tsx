...
function SearchInterface() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const resultCount = selectedTab === 'courses' 
        ? searchHook.data?.total || 0
        : lessonSearchHook.data?.total || 0;
  ...
  const handleResultClick = useCallback((result: CourseSearchResult, position: number) => {
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600">
          Find the perfect course from our extensive library
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        {/* Results List */}
        <div className={selectedTab === 'courses' && searchHook.data?.facets ? 'lg:col-span-3' : 'lg:col-span-4'}>
          {selectedTab === 'courses' ? (
            <CourseResultsList 
              results={searchHook.data?.documents || []}
              loading={searchHook.loading}
              onResultClick={handleResultClick}
              onLoadMore={() => searchHook.fetchMore({ from: searchHook.data?.documents.length || 0, size: 20 })}
            ...
            <LessonResultsList 
              ...
              onLoadMore={() => lessonSearchHook.fetchMore({ from: lessonSearchHook.data?.documents.length || 0, size: 20 })}
...
function CourseResultsList({ results, loading, onResultClick, onLoadMore, hasMore }: CourseResultsListProps) {
        ...
        >
          ...
          {result.highlight && (
            <div className="mt-3 text-sm text-gray-600">
              <div dangerouslySetInnerHTML={{ __html: Object.values(result.highlight)[0] as string }} />
            </div>
          )}
        </div>
      ))}
