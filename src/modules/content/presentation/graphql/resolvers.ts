...
import { validateFileSize, MAX_FILE_SIZES } from '../../../../shared/utils/validation.js';
...
function mapVideoAssetToGraphQL(videoAsset: VideoAsset): VideoAssetGraphQL {
  return {
    ...
    // Computed fields
    formattedDuration: videoAsset.durationSeconds
      ? formatDuration(videoAsset.durationSeconds)
      : null,
    formattedFileSize: formatFileSize(videoAsset.originalFileSize),
    isProcessing:
      videoAsset.processingStatus === 'in_progress' || videoAsset.processingStatus === 'pending',
    ...
    processingProgress:
      videoAsset.processingStatus === 'completed'
        ? 100
        : videoAsset.processingStatus === 'failed'
          ? 0
          : 50,
...
function mapFileAssetToGraphQL(fileAsset: FileAsset): FileAssetGraphQL {
  return {
    ...
    isProcessing:
      fileAsset.processingStatus === 'in_progress' || fileAsset.processingStatus === 'pending',
    ...
    timeUntilExpiration: fileAsset.expiresAt
      ? Math.max(0, fileAsset.expiresAt.getTime() - Date.now())
      : null,
    isExpiringSoon: fileAsset.expiresAt
      ? fileAsset.expiresAt.getTime() - Date.now() < 24 * 60 * 60 * 1000
      : false,
...
export const contentResolvers = {
  Query: {
    ...
    ): Promise<VideoAssetGraphQL | null> => {
      requireAuth(context);
      ...
      try {
        ...
        return mapVideoAssetToGraphQL(
          new VideoAsset(
            ...
            Array.isArray(videoAsset.availableResolutions)
              ? (videoAsset.availableResolutions as VideoResolution[])
              : [],
     ...
     */
    generateStreamingUrl: async (
      _parent: unknown,
      args: { lessonId: string; resolution?: string; format?: string },
      context: GraphQLContext
    ): Promise<{ streamingUrl: string; expiresAt: Date; resolution: string; format: string }> => {
  ...
  Mutation: {
    ...
    generateUploadUrl: async (
      _parent: unknown,
      args: { input: GenerateUploadUrlInput },
      context: GraphQLContext
    ): Promise<{ uploadUrl: string; fileKey: string; expiresIn: number; maxFileSize: number }> => {
      const authUser = requireAuth(context);
      ...
      try {
        ...
        return {
          ...
          maxFileSize:
            args.input.fileType === 'video' ? MAX_FILE_SIZES.video : MAX_FILE_SIZES.document,
        };
      } catch (error) {
        if (error instanceof GraphQLError) {
          throw error;
    ...
    uploadCourseResource: (
      _parent: unknown,
      _args: { input: UploadCourseResourceInput },
