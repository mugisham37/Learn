...
export async function registerAuthRoutes(
  ...
  fastify.post<{ Body: LoginRequest }>('/api/auth/login', {
    ...
    try {
      ...
      const csrfToken = (await import('../../../../shared/middleware/csrf.js')).generateCSRFToken();
  ...
  // Refresh token
  fastify.post<{ Body: RefreshTokenRequest }>('/api/auth/refresh', {
    schema: {
      body: {
        type: 'object',
        properties: {
  ...
  fastify.post('/api/auth/logout', {
    ...
    try {
      ...
      logger.error(
        { requestId, userId: user.userId, error: error instanceof Error ? error.message : 'Unknown error' },
        'Logout failed'
      );
  ...
  // Verify email
  fastify.post<{ Body: VerifyEmailRequest }>('/api/auth/verify-email', {
    schema: {
      body: {
        type: 'object',
        required: ['token'],
  ...
  fastify.post<{ Body: RequestPasswordResetRequest }>('/api/auth/request-password-reset', {
    schema: {
      body: {
        type: 'object',
        required: ['email'],
    ...
    try {
      ...
      logger.info(
        { requestId, email: request.body.email },
  ...
  // Reset password
  fastify.post<{ Body: ResetPasswordRequest }>('/api/auth/reset-password', {
    schema: {
      body: {
        type: 'object',
