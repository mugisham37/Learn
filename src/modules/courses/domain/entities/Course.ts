...
export class Course {
  ...
  get difficulty(): CourseDifficulty { return this._props.difficulty; }
  get price(): number { return this._props.price; }
  get currency(): string { return this._props.currency; }
  get enrollmentLimit(): number | undefined { return this._props.enrollmentLimit; }
  get enrollmentCount(): number { return this._props.enrollmentCount; }
  get averageRating(): number | undefined { return this._props.averageRating; }
  get totalReviews(): number { return this._props.totalReviews; }
  get status(): CourseStatus { return this._props.status; }
  ...
  get modules(): CourseModule[] { return [...this._modules]; }
  get domainEvents(): any[] { return [...this._domainEvents]; }

  // Static factory method for creating new courses
  static create(props: Omit<CourseProps, 'id' | 'slug' | 'enrollmentCount' | 'totalReviews' | 'status' | 'createdAt' | 'updatedAt'>): Course {
    const now = new Date();
  ...
  addModule(module: CourseModule): void {
    ...
    this.addDomainEvent(new ModuleAddedEvent(
      this.id,
      module.id,
      module.title,
      module.orderNumber
  ...
  canPublish(): { canPublish: boolean; reasons: string[] } {
    ...
    const modulesWithoutContent = this._modules.filter(module => 
