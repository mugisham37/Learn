...
import { ICourseService, PublicationValidationResult } from '../../application/services/ICourseService.js';
 ...
 */
function mapDifficultyFromGraphQL(difficulty: 'BEGINNER' | 'INTERMEDIATE' | 'ADVANCED'): 'beginner' | 'intermediate' | 'advanced' {
  switch (difficulty) {
    case 'BEGINNER':
      return 'beginner';
    case 'INTERMEDIATE':
...
function mapLessonTypeFromGraphQL(type: 'VIDEO' | 'TEXT' | 'QUIZ' | 'ASSIGNMENT'): 'video' | 'text' | 'quiz' | 'assignment' {
  switch (type) {
    case 'VIDEO':
      return 'video';
    case 'TEXT':
...
export const courseResolvers = {
  Query: {
     ...
     * Get course by ID
     */
    course: async (_parent: any, args: { id: string }, context: GraphQLContext): Promise<Course | null> => {
      try {
     ...
     * Get course by slug
     */
    courseBySlug: async (_parent: any, args: { slug: string }, context: GraphQLContext): Promise<Course | null> => {
      try {
     ...
     */
    courses: async (_parent: any, args: { filter?: CourseFilter; pagination?: PaginationInput }, context: GraphQLContext) => {
      try {
     ...
     */
    myCourses: async (_parent: any, args: { filter?: CourseFilter; pagination?: PaginationInput }, context: GraphQLContext) => {
      ...
      try {
        ...
        const result = await context.courseService.getCoursesByInstructor(user.id, pagination, filters);
     ...
     */
    validateCoursePublication: async (_parent: any, args: { id: string }, context: GraphQLContext): Promise<PublicationValidationResult> => {
      const user = requireEducator(context);
  ...
  Mutation: {
     ...
     * Create a new course
     */
    createCourse: async (_parent: any, args: { input: CreateCourseInput }, context: GraphQLContext): Promise<Course> => {
      const user = requireEducator(context);
     ...
     * Update course
     */
    updateCourse: async (_parent: any, args: { id: string; input: UpdateCourseInput }, context: GraphQLContext): Promise<Course> => {
      const user = requireEducator(context);
      ...
      try {
        const updateData: UpdateCourseDTO = {
          ...
          description: args.input.description?.trim(),
          category: args.input.category?.trim(),
          difficulty: args.input.difficulty ? mapDifficultyFromGraphQL(args.input.difficulty) : undefined,
     ...
     * Publish course
     */
    publishCourse: async (_parent: any, args: { id: string }, context: GraphQLContext): Promise<Course> => {
      const user = requireEducator(context);
     ...
     * Delete course
     */
    deleteCourse: async (_parent: any, args: { id: string }, context: GraphQLContext): Promise<boolean> => {
      const user = requireEducator(context);
     ...
     */
    addModule: async (_parent: any, args: { courseId: string; input: CreateModuleInput }, context: GraphQLContext): Promise<CourseModule> => {
      const user = requireEducator(context);
     ...
     */
    addLesson: async (_parent: any, args: { moduleId: string; input: CreateLessonInput }, context: GraphQLContext): Promise<Lesson> => {
      const user = requireEducator(context);
     ...
     * Reorder modules
     */
    reorderModules: async (_parent: any, args: { courseId: string; input: ReorderModulesInput }, context: GraphQLContext): Promise<CourseModule[]> => {
      const user = requireEducator(context);
      ...
      try {
        ...
        const modules = await context.courseService.reorderModules(args.courseId, user.id, args.input.moduleIds);
        return modules;
      } catch (error: any) {
        if (error instanceof GraphQLError) {
          throw error;
        ...
        if (error.message?.includes('validation') || error.message?.includes('count') || error.message?.includes('belong')) {
     ...
     * Update module
     */
    updateModule: async (_parent: any, args: { id: string; input: UpdateModuleInput }, context: GraphQLContext): Promise<CourseModule> => {
      const user = requireEducator(context);
      
      try {
        // Validate input
        if (args.input.title !== undefined && (!args.input.title || args.input.title.trim().length === 0)) {
     ...
     * Delete module
     */
    deleteModule: async (_parent: any, args: { id: string }, context: GraphQLContext): Promise<boolean> => {
      const user = requireEducator(context);
     ...
     * Update lesson
     */
    updateLesson: async (_parent: any, args: { id: string; input: UpdateLessonInput }, context: GraphQLContext): Promise<Lesson> => {
      const user = requireEducator(context);
     ...
     * Delete lesson
     */
    deleteLesson: async (_parent: any, args: { id: string }, context: GraphQLContext): Promise<boolean> => {
      const user = requireEducator(context);
     ...
     * Reorder lessons
     */
    reorderLessons: async (_parent: any, args: { moduleId: string; input: ReorderLessonsInput }, context: GraphQLContext): Promise<Lesson[]> => {
      const user = requireEducator(context);
