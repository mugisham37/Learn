...
export class CourseDataLoaders {
   ...
   */
  private async batchLoadCoursesByInstructorIds(instructorIds: string[]): Promise<Map<string, Course[]>> {
    const coursesMap = new Map<string, Course[]>();
   ...
   */
  private async batchLoadModulesByCourseIds(courseIds: string[]): Promise<Map<string, CourseModule[]>> {
    ...
    for (const { courseId, modules } of results) {
      const domainModules = modules.map(moduleSchema => new CourseModule({
        id: moduleSchema.id,
  ...
  private async batchLoadLessonsByModuleIds(moduleIds: string[]): Promise<Map<string, Lesson[]>> {
    ...
    for (const { moduleId, lessons } of results) {
      const domainLessons = lessons.map(lessonSchema => new Lesson({
        id: lessonSchema.id,
