 ...
 */
function requireAuth(context: AssessmentGraphQLContext): { id: string; email: string; role: string } {
  if (!context.user) {
 ...
 */
function requireEducator(context: AssessmentGraphQLContext): { id: string; email: string; role: string } {
 ...
 */
function requireStudent(context: AssessmentGraphQLContext): { id: string; email: string; role: string } {
  const user = requireAuth(context);
 ...
 */
function mapQuizTypeFromGraphQL(type: 'FORMATIVE' | 'SUMMATIVE' | 'PRACTICE'): 'formative' | 'summative' | 'practice' {
  switch (type) {
    case 'FORMATIVE':
      return 'formative';
    case 'SUMMATIVE':
 ...
 */
function mapQuestionTypeFromGraphQL(type: string): 'multiple_choice' | 'true_false' | 'short_answer' | 'essay' | 'fill_blank' | 'matching' {
  switch (type) {
 ...
 */
function mapDifficultyFromGraphQL(difficulty: 'EASY' | 'MEDIUM' | 'HARD'): 'easy' | 'medium' | 'hard' {
  switch (difficulty) {
    case 'EASY':
      return 'easy';
    case 'MEDIUM':
...
export const assessmentResolvers = {
  Query: {
     ...
     * Get quiz by ID
     */
    quiz: async (_parent: unknown, args: { id: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireAuth(context);
     ...
     */
    quizzesByLesson: async (_parent: unknown, args: { lessonId: string }, context: AssessmentGraphQLContext): Promise<unknown[]> => {
      requireAuth(context);
      
      try {
        const result = await context.quizRepository.findByLesson(args.lessonId, { page: 1, limit: 100 });
        return result.data;
      } catch (error) {
        handleServiceError(error);
        return [];
     ...
     * Get question by ID
     */
    question: async (_parent: unknown, args: { id: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireAuth(context);
     ...
     */
    questionsByQuiz: async (_parent: unknown, args: { quizId: string }, context: AssessmentGraphQLContext): Promise<unknown[]> => {
      requireAuth(context);
     ...
     */
    quizSubmission: async (_parent: unknown, args: { id: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireAuth(context);
     ...
     */
    quizSubmissionsByStudent: async (_parent: unknown, args: { quizId: string }, context: AssessmentGraphQLContext): Promise<unknown[]> => {
      const user = requireAuth(context);
      
      try {
        const submissions = await context.quizSubmissionRepository.findAllAttempts(args.quizId, user.id);
        return submissions;
      } catch (error) {
        handleServiceError(error);
        return [];
     ...
     */
    quizSubmissionsByQuiz: async (_parent: unknown, args: { quizId: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireEducator(context);
      
      try {
        const result = await context.quizSubmissionRepository.findByQuiz(args.quizId, { page: 1, limit: 100 });
        return result.data;
      } catch (error) {
        handleServiceError(error);
        return [];
     ...
     */
    assignment: async (_parent: unknown, args: { id: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireAuth(context);
      
      try {
        const assignment = await context.assignmentService.getAssignment(args.id, user.id, user.role);
        return assignment;
      } catch (error) {
        handleServiceError(error);
        return null;
     ...
     */
    assignmentsByLesson: async (_parent: unknown, args: { lessonId: string }, context: AssessmentGraphQLContext): Promise<unknown[]> => {
      requireAuth(context);
      
      try {
        const result = await context.assignmentRepository.findByLesson(args.lessonId, { page: 1, limit: 100 });
        return result.data;
      } catch (error) {
        handleServiceError(error);
        return [];
     ...
     */
    assignmentSubmission: async (_parent: unknown, args: { id: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireAuth(context);
      
      try {
        const submission = await context.assignmentService.getSubmission(args.id, user.id, user.role);
        return submission;
      } catch (error) {
        handleServiceError(error);
        return null;
     ...
     */
    assignmentSubmissionsByStudent: async (_parent: unknown, args: { assignmentId: string }, context: AssessmentGraphQLContext): Promise<unknown[]> => {
      const user = requireAuth(context);
      
      try {
        const result = await context.assignmentSubmissionRepository.findByStudent(user.id, { page: 1, limit: 100 }, { assignmentId: args.assignmentId });
        return result.data;
      } catch (error) {
        handleServiceError(error);
        return [];
     ...
     */
    assignmentSubmissionsByAssignment: async (_parent: unknown, args: { assignmentId: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireEducator(context);
      
      try {
        const result = await context.assignmentSubmissionRepository.findByAssignment(args.assignmentId, { page: 1, limit: 100 });
        return result.data;
      } catch (error) {
        handleServiceError(error);
        return [];
  ...
  Mutation: {
     ...
     */
    createQuiz: async (_parent: unknown, args: { input: CreateQuizInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      ...
      try {
        const quiz = await context.quizService.createQuiz(user.id, {
          ...
          availableFrom: args.input.config.availableFrom ? new Date(args.input.config.availableFrom) : undefined,
          availableUntil: args.input.config.availableUntil ? new Date(args.input.config.availableUntil) : undefined
        });
     ...
     */
    updateQuiz: async (_parent: unknown, _args: { id: string; input: UpdateQuizInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireEducator(context);
     ...
     */
    deleteQuiz: async (_parent: unknown, _args: { id: string }, context: AssessmentGraphQLContext): Promise<boolean> => {
      requireEducator(context);
     ...
     */
    addQuestion: async (_parent: unknown, args: { input: CreateQuestionInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireEducator(context);
     ...
     */
    updateQuestion: async (_parent: unknown, _args: { id: string; input: UpdateQuestionInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireEducator(context);
     ...
     */
    deleteQuestion: async (_parent: unknown, _args: { id: string }, context: AssessmentGraphQLContext): Promise<boolean> => {
      requireEducator(context);
     ...
     */
    startQuizAttempt: async (_parent: unknown, args: { quizId: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
     ...
     */
    submitQuizAnswer: async (_parent: unknown, args: { submissionId: string; input: SubmitQuizAnswerInput }, context: AssessmentGraphQLContext): Promise<boolean> => {
     ...
     */
    submitQuiz: async (_parent: unknown, args: { submissionId: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireStudent(context);
     ...
     */
    gradeQuizSubmission: async (_parent: unknown, args: { submissionId: string; input: GradeQuizInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireEducator(context);
     ...
     */
    createAssignment: async (_parent: unknown, args: { input: CreateAssignmentInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireEducator(context);
     ...
     */
    updateAssignment: async (_parent: unknown, _args: { id: string; input: UpdateAssignmentInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      requireEducator(context);
     ...
     */
    deleteAssignment: async (_parent: unknown, _args: { id: string }, context: AssessmentGraphQLContext): Promise<boolean> => {
      requireEducator(context);
     ...
     */
    submitAssignment: async (_parent: unknown, args: { input: SubmitAssignmentInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireStudent(context);
     ...
     */
    gradeAssignment: async (_parent: unknown, args: { submissionId: string; input: GradeAssignmentInput }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireEducator(context);
     ...
     */
    requestRevision: async (_parent: unknown, args: { submissionId: string; feedback: string }, context: AssessmentGraphQLContext): Promise<unknown> => {
      const user = requireEducator(context);
