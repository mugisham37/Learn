...
export class QueueManager {
  ...
  public async getHealthStatus(): Promise<{
    ...
    try {
      ...
      const healthy = alerts.filter(alert => 
      ...
      return {
        healthy: false,
        queues: [],
        alerts: [{ message: 'Health check failed', error: error instanceof Error ? error.message : String(error) }],
        timestamp: new Date(),
      };
    }
  }
  ...
  private registerEventListeners(): void {
    ...
    queueTypes.forEach(queueType => {
      this.queueFactory!.registerEventListener(
        queueType,
        this.queueMonitor!
      );
    });
