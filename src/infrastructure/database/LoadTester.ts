...
export class DatabaseLoadTester {
  ...
  public async runLoadTest(config: LoadTestConfig): Promise<LoadTestResult> {
    console.log(`Starting load test with ${config.concurrentConnections} concurrent connections for ${config.testDurationMs}ms`);
    ...
    const metricsInterval = setInterval(() => {
      const writeUtilization = (this.writePool.totalCount / (this.writePool.options.max || 20)) * 100;
    ...
    if (poolMetrics.length > 0) {
      const avgWriteUtil = poolMetrics.reduce((sum, m) => sum + m.writeUtilization, 0) / poolMetrics.length;
      const avgReadUtil = poolMetrics.reduce((sum, m) => sum + m.readUtilization, 0) / poolMetrics.length;
      ...
      const avgWriteWaiting = poolMetrics.reduce((sum, m) => sum + m.writeWaiting, 0) / poolMetrics.length;
      const avgReadWaiting = poolMetrics.reduce((sum, m) => sum + m.readWaiting, 0) / poolMetrics.length;
    ...
    console.log(`Load test completed: ${result.totalQueries} queries, ${result.queriesPerSecond.toFixed(2)} QPS, ${result.failedQueries} failures`);
  ...
  private generateRecommendations(result: LoadTestResult): string[] {
    ...
    if (result.failedQueries / result.totalQueries > 0.05) {
      recommendations.push(`High failure rate (${((result.failedQueries / result.totalQueries) * 100).toFixed(1)}%) - consider increasing pool sizes or optimizing queries`);
    }
    
    // High pool utilization
    if (result.peakPoolUtilization > 90) {
      recommendations.push(`Peak pool utilization is ${result.peakPoolUtilization.toFixed(1)}% - consider increasing max pool size`);
    }
    
    // High average utilization
    if (result.averagePoolUtilization > 70) {
      recommendations.push(`Average pool utilization is ${result.averagePoolUtilization.toFixed(1)}% - monitor for sustained high load`);
    }
    
    // Connection timeouts
    if (result.connectionTimeouts > 0) {
      recommendations.push(`${result.connectionTimeouts} connection timeouts detected - increase pool size or connection timeout`);
    }
    
    // High waiting clients
    if (result.peakWaitingClients > 10) {
      recommendations.push(`Peak waiting clients: ${result.peakWaitingClients} - pool may be undersized for this load`);
    }
    
    // Slow queries
    if (result.p95QueryTime > 1000) {
      recommendations.push(`95th percentile query time is ${result.p95QueryTime.toFixed(0)}ms - investigate slow queries`);
    }
    
    // Low throughput
    const expectedQPS = result.config.concurrentConnections * (1000 / Math.max(result.config.queryInterval, 1));
    if (result.queriesPerSecond < expectedQPS * 0.8) {
      recommendations.push(`Actual QPS (${result.queriesPerSecond.toFixed(1)}) is below expected (${expectedQPS.toFixed(1)}) - check for bottlenecks`);
    }
  ...
  public async quickStressTest(): Promise<{
    ...
    console.log(`Stress test completed: ${results.maxConcurrentConnections} max connections, ${results.connectionEstablishmentTime.toFixed(2)}ms establishment time`);
