...
export class ApplicationMetricsService extends EventEmitter implements IApplicationMetricsService {
  ...
  private databaseQueries: Array<{
    operation: string;
    table: string;
    duration: number;
    timestamp: number;
  }> = [];
  private externalServiceCalls: Array<{
    service: string;
    operation: string;
    duration: number;
    success: boolean;
    ...
    try {
      ...
      if (responseTimeMetrics.count > 0) {
        await cloudWatchService.putMetric(
          'ResponseTimeP50',
          responseTimeMetrics.p50,
          'Milliseconds'
        );
        await cloudWatchService.putMetric(
          'ResponseTimeP95',
          responseTimeMetrics.p95,
          'Milliseconds'
        );
        await cloudWatchService.putMetric(
          'ResponseTimeP99',
          responseTimeMetrics.p99,
          'Milliseconds'
        );
      }
      ...
      await cloudWatchService.putMetric(
        'RequestsPerSecond',
        throughputMetrics.requestsPerSecond,
        'Count/Second'
      );
      ...
      if (dbMetrics.queryCount > 0) {
        ...
        await cloudWatchService.putMetric(
          'DatabaseAverageQueryTime',
          dbMetrics.averageDuration,
          'Milliseconds'
        );
      ...
      if (extMetrics.callCount > 0) {
        ...
        await cloudWatchService.putMetric(
          'ExternalServiceAverageLatency',
          extMetrics.averageDuration,
          'Milliseconds'
        );
      ...
      await cloudWatchService.putMetric(
        'MemoryUsageHeapUsed',
        resourceMetrics.memoryUsage.heapUsed,
        'Bytes'
      );
      await cloudWatchService.putMetric(
        'MemoryUsageHeapTotal',
        resourceMetrics.memoryUsage.heapTotal,
        'Bytes'
      );
