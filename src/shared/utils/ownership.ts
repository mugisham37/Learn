 ...
 */
export async function verifyCourseOwnership(userId: string, courseId: string): Promise<boolean> {
  try {
    const db = getReadDb();
 ...
 */
export async function verifyMessageOwnership(userId: string, messageId: string): Promise<boolean> {
  try {
    const db = getReadDb();
