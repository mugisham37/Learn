...
export function registerStaticAssetEndpoint(fastify: FastifyInstance): void {
  ...
  fastify.get('/static/*', {
    preHandler: [
      createHttpCachingMiddleware(CacheConfigs.STATIC_ASSETS),
    ],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
...
export function registerHealthCheckEndpoint(fastify: FastifyInstance): void {
  ...
  fastify.get('/api/health/status', {
    preHandler: [
      createHttpCachingMiddleware(CacheConfigs.NO_CACHE),
    ],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    return {
      status: 'healthy',
...
export function registerConditionalRequestExample(fastify: FastifyInstance): void {
  ...
  fastify.get('/api/example/conditional', {
    preHandler: [
      createHttpCachingMiddleware(CacheConfigs.API_RESPONSES),
    ],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
